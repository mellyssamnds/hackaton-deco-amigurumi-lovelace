...
(`WATERMARK_DISPATCH_MAX_RETRIES`, default 3), erros 4xx (payload
rejeitado pela Parte 3) não são retentados. Qualquer falha depois de
esgotar as tentativas propaga `WatermarkDispatchError`, que o
`QueueConsumer` trata como falha de infraestrutura: a mensagem volta para
retry na fila (não é descartada).
